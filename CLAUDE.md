...
Coverage uses the V8 provider and remaps bundle coverage back to `src/` via source maps; thresholds are scoped to `./src/` so vendored dependencies (delaunator, robust-predicates, chroma-js in the debug bundle) don't dilute or inflate them.
